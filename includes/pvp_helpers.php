...
try {
    ...
    // Tabela chat meciuri (efemer - se șterge după finalizare)
    ...
    // Inserăm ligile default dacă nu există
...
/**
 * Calculează scorul de popularitate pentru un user
 */
function calculatePopularityScore($userId) {
    ...
    // Vizite la fermă (2 puncte/vizită) - ultimele 7 zile
    ...
    // Interacțiuni generale (1 punct) - mesaje trimise
...
/**
 * Verifică dacă sunt suficienți jucători pentru a începe un battle
 */
...
/**
 * Alocă random 64 jucători pentru un battle
 */
function allocatePlayers($battleId, $leagueId) {
    global $db;
    
    // Selectăm 64 jucători random din liga respectivă
    $stmt = $db->prepare("
    ...
    // Calculăm și salvăm scorul de popularitate pentru fiecare
...
/**
 * Creează meciurile pentru prima rundă (1/32 - 64 jucători)
 */
function createFirstRoundMatches($battleId) {
    global $db;
    
    // Luăm toți participanții sortați aleatoriu
    ...
    // Creăm 32 meciuri (64 jucători / 2)
...
/**
 * Determină câștigătorii unei runde și avansează la următoarea
 */
function processRoundResults($battleId, $roundNumber) {
    global $db;
    
    // Luăm toate meciurile rundei
    ...
    // Dacă mai sunt câștigători, creăm runda următoare
    if (count($winners) > 1) {
...
/**
 * Promovează top 4 jucători (semifinaliști + finaliști) la liga superioară
 */
function promoteTopPlayers($battleId) {
    global $db;
    
    // Găsim semifinala și finala
    ...
    // Top 4: participanții din semifinale
    $stmt = $db->prepare("
...
/**
 * Reset lunar - redistribuie toți jucătorii
 */
function monthlyLeagueReset() {
    ...
    // Verificăm dacă s-a făcut deja reset luna asta
    $stmt = $db->query("SELECT MAX(last_reset_date) FROM user_league_status");
    $lastReset = $stmt->fetchColumn();
    
    if ($lastReset && $lastReset >= $today) {
        return false; // Deja făcut
    }
    
    // Resetăm toți userii la liga bronz
...
/**
 * Obține status-ul battle-ului curent pentru un user
 */
function getUserBattleStatus($userId) {
    ...
    // Găsim battle-ul activ pentru liga sa
    ...
    // Verificăm dacă userul participă
    ...
    // Găsim meciul curent al userului
    $stmt = $db->prepare("
...
/**
 * Obține numărul de mesaje necitite în meciul userului
 */
function getUnreadMatchChatCount($userId) {
    ...
    // Numărăm mesajele necitite trimise de adversar
    $stmt = $db->prepare("
...
/**
 * Marchează mesajele dintr-un meci ca citite pentru un user
 */
