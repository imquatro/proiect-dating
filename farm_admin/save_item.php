...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $db->prepare('INSERT INTO farm_items (name,item_type,slot_type,image_plant,image_product,water_interval,feed_interval,water_times,feed_times,production,price,barn_capacity) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)');
    $stmt->execute([$name,$item_type,$slot_type,$imgPlant,$imgProduct,$water_interval,$feed_interval,$water_times,$feed_times,$production,$price,$barn_capacity]);
