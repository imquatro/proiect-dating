...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $water_interval = intval($_POST['water_interval'] ?? 0);
    $feed_interval = intval($_POST['feed_interval'] ?? 0);
    ...
    $stmt = $db->prepare('INSERT INTO farm_items (name,item_type,slot_type,image_plant,image_ready,image_product,water_interval,feed_interval,water_times,feed_times,production) VALUES (?,?,?,?,?,?,?,?,?,?,?)');
    $stmt->execute([$name,$item_type,$slot_type,$imgPlant,$imgReady,$imgProduct,$water_interval,$feed_interval,$water_times,$feed_times,$production]);
