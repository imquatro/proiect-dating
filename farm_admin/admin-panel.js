...
function initDeleteItems(panel){
    const items = panel.querySelectorAll('#fa-tab-delete .fa-delete-item');
    const delBtn = panel.querySelector('#fa-delete-item-btn');
    if (!items.length || !delBtn) return;
    let selectedId = null;
    ...
    delBtn.addEventListener('click', () => {
        ...
        .then(data => {
            if (data.success) {
                const delItem = panel.querySelector(`#fa-tab-delete .fa-delete-item[data-id="${selectedId}"]`);
